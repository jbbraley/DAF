...
kt = [1.2106E+04 2.4354E+04 9.0220E+03 8.8507E+04 2.8893E+05 1.1706E+06 8.8507E+04]; % suspension stiffness for each vehicle
kb = []; % stiffness of bridge (idealized as sprung mass)
ct = []; % damping coefficient of truck
cb = []; % damping coefficient of bridge
mt = [119.14 119.14 51.80 119.14 119.14 119.14 119.14]; % mass of each vehicle
mb = []; % effective mass of bridge
...
for bridge = 1:length(kb)
    ...
    for vehicle = 1:length(ks)
        truck.kt = kt(vehicle); % lb/in
        truck.ct = ct(vehicle);            % lb.s/in
        truck.mt = mt(vehicle)*386.09;  % vehicle weight
