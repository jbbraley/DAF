classdef sgl_dc < vb_base
%% classdef sgl_dc
% 
% 
% 
% author: John Braley
% create date: 03-Apr-2019 10:32:36
...
%% dependent methods
	methods 
       ...
       function ssmodel = get.ssmodel(self)
           ...
           A = @(t) [-self.db*(2*pi*self.fnb) -pi^4*self.EI/(self.mb/self.gravity*self.L^3) 2*self.ct/(self.mb/self.gravity)*(sin(pi*self.vel*t/self.L)) 2*self.kt/(self.mb/self.gravity)*(sin(pi*self.vel*t/self.L));...
               1 0 0 0;...
