...
freq_ub = 0.25; % lower bound for frequency band of included frequency content

%% Generate profile
% Construct amplitudes and phase angles for each spatial frequency
% n  = [4*dn:dn:(freq_lb-dn) (freq_ub+dn):dn:N*dn] ; % Spatial Frequency Band (start higher in frequency to avoid large global elevation changes)
n  = dn:dn:N*dn ; % Spatial Frequency Band (start higher in frequency to avoid large global elevation changes)
sub_ind = find(n==freq_lb,1,'first'):find((n-freq_ub)>0,1,'first'); %[1:(find(n==freq_lb,1,'first')-1) (find(n==freq_ub,1,'first')+1):length(n)];
n_sub = n(sub_ind); %
nangle = n_sub*2*pi; % angular frequency band
psd = C*n_sub.^(-w); % fitted psd value for each frequency band
...
save_file = file();
save_file.name = ['ISO_e' num2str(14.81) 'w' num2str(2.495) '_LB-' num2str(freq_ub) '.csv'];
